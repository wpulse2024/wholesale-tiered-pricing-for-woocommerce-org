...
class WHTPRole_Registration {
	...
	public function render_shortcode( $atts ) {
		...
		$form_title = ! empty( $atts['title'] ) ? $atts['title'] : ( ! empty( $settings['form_title'] ) ? $settings['form_title'] : __( 'Apply for Wholesale Access', 'wholesale-tiered-pricing-for-woocommerce' ) );
	...
	public function ajax_submit_application() {
		...
		$errors = array();
		if ( empty( $company ) ) {
			$errors[] = __( 'Company name is required.', 'wholesale-tiered-pricing-for-woocommerce' );
		}
		if ( empty( $phone ) ) {
			$errors[] = __( 'Phone number is required.', 'wholesale-tiered-pricing-for-woocommerce' );
		}
		if ( empty( $business_type ) ) {
			$errors[] = __( 'Business type is required.', 'wholesale-tiered-pricing-for-woocommerce' );
		}
		...
		if ( is_user_logged_in() ) {
			...
			if ( empty( $first_name ) ) {
				$errors[] = __( 'First name is required.', 'wholesale-tiered-pricing-for-woocommerce' );
			}
			if ( empty( $last_name ) ) {
				$errors[] = __( 'Last name is required.', 'wholesale-tiered-pricing-for-woocommerce' );
			}
			if ( empty( $email ) || ! is_email( $email ) ) {
				$errors[] = __( 'A valid email address is required.', 'wholesale-tiered-pricing-for-woocommerce' );
			}
	...
	public function ajax_approve_application() {
		...
		$settings      = $this->get_settings();
		$approval_role = ! empty( $settings['approval_role'] ) ? $settings['approval_role'] : $this->get_default_approval_role();
	...
	private function send_admin_notification( $user_id ) {
		...
		$user      = get_userdata( $user_id );
		$decoded   = json_decode( get_user_meta( $user_id, '_whtprole_application_data', true ), true );
		$app_data  = is_array( $decoded ) ? $decoded : array();
