...
class WHTPRole_Pricing_Helper {
	...
	public function is_valid_product_to_applied_tiered_pricing( $product_id ) {
		$global_product_settings = get_option( 'whtprole_global_product_settings', array() );
		$global_product_settings = is_array( $global_product_settings ) ? $global_product_settings : json_decode( $global_product_settings, true );
		$include_products        = ! empty( $global_product_settings['include_products'] ) ? $global_product_settings['include_products'] : array();
		$exclude_products        = ! empty( $global_product_settings['exclude_products'] ) ? $global_product_settings['exclude_products'] : array();
		$apply_type              = ! empty( $global_product_settings['apply_type'] ) ? $global_product_settings['apply_type'] : 'include';
	...
	public function is_valid_category_to_applied_tiered_pricing( $product_id ) {
		...
		$global_category_settings = get_option( 'whtprole_global_product_settings', array() );
		$global_category_settings = is_array( $global_category_settings ) ? $global_category_settings : json_decode( $global_category_settings, true );

		$include_categories = ! empty( $global_category_settings['include_categories'] ) ? $global_category_settings['include_categories'] : array();
		$exclude_categories = ! empty( $global_category_settings['exclude_categories'] ) ? $global_category_settings['exclude_categories'] : array();
		$apply_type         = ! empty( $global_category_settings['apply_type'] ) ? $global_category_settings['apply_type'] : 'include';

		if ( $apply_type === 'include' ) {
			if ( ! empty( $include_categories ) && ! array_intersect( $include_categories, $product_category_ids ) ) {
				return false;
			}
		} else {
			if ( ! empty( $exclude_categories ) && array_intersect( $exclude_categories, $product_category_ids ) ) {
				return false;
			}
		}
		return true;
	...
	public function get_general_settings() {
		...
		$raw = get_option( 'whtprole_pricing_save_general_settings', array() );
		if ( is_array( $raw ) ) {
			self::$general_settings_cache = $raw;
		} else {
			$decoded                      = json_decode( $raw, true );
			self::$general_settings_cache = is_array( $decoded ) ? $decoded : array();
		}
		return self::$general_settings_cache;
	}

	public function validation( $product_id ) {
		if ( ! $this->is_valid_product_to_applied_tiered_pricing( $product_id ) ) {
			return false;
		}
		if ( ! $this->is_valid_category_to_applied_tiered_pricing( $product_id ) ) {
			return false;
		}
	...
	public function enable_to_shows_table( $product_id ) {
		$show_table      = get_post_meta( $product_id, '_show_pricing_table', true );
		$global_settings = $this->get_general_settings();

		if ( ! empty( $show_table ) && $show_table === 'no' ) {
			return false;
		}

		if ( empty( $show_table ) && is_array( $global_settings ) && isset( $global_settings['showTieredPricing'] ) && $global_settings['showTieredPricing'] === false ) {
			return false;
		}
	...
	public function get_template_path() {
		$global_settings = $this->get_general_settings();
		$template        = ! empty( $global_settings['defaultTemplate'] ) ? $global_settings['defaultTemplate'] : 'table';
	...
	public function calculation_discount( $regular_price, $tier ) {
	...
	public function get_tiered_featured( $tiered_pricing, $regular_price ) {
		$discounted_tier = 0;
		// FIX [LOW-05]: was 1 — any tier with <1% discount was never selected.
		$discount_amount = 0;
		foreach ( $tiered_pricing as $index => $tier ) {
			$discount        = $this->calculation_discount( $regular_price, $tier );
			$savings_percent = $discount['savings_percent'];
			if ( $savings_percent > $discount_amount ) {
				$discounted_tier = $index;
				$discount_amount = $savings_percent;
			}
		}
	...
	public static function calculate_price( float $base_price, array $rule, int $quantity, ?int $variation_id = null ): float {
		...
		$valid_discount_types = array( 'fixed', 'percentage' );
