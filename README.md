...
- Moved activation hook registration to plugin load time for correct WordPress lifecycle behavior.
- Renamed global `get_wp_user_roles` function to a prefixed name to avoid namespace collision with other plugins.
- Removed dead code: unused `find_applicable_tier()` and `getPrice()` methods, stray second class instantiation that caused all hooks to register twice, and commented-out block.
